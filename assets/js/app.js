...
var afterAllInjectionsFinishedCallback = function afterAllInjectionsFinishedCallback(totalSVGsInjected) {// Callback after all SVGs are injected
  //console.log('We injected ' + totalSVGsInjected + ' SVG(s)!');
};

var perInjectionCallback = function perInjectionCallback(svg) {// Callback after each SVG is injected
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(function () {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).trigger('enhance.tablesaw');
});
var TablesawConfig = {
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()('#heroVideo').on('open.zf.reveal', function () {
  player.playVideo();
});
jquery__WEBPACK_IMPORTED_MODULE_0___default()('#heroVideo').on('closed.zf.reveal', function () {
  player.pauseVideo();
}); // Loading Button

jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-loading-start]').click(function () {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).addClass('hide');
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).parent().find('[data-loading-end]').removeClass('hide');
  setTimeout(function () {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-loading-start]').removeClass('hide');
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-loading-end]').addClass('hide');
  }, 2000);
}); // Program Filter (src: https://foundation.zurb.com/building-blocks/blocks/filter-simple.html)
// Retrieve the Course Filter from URL parameter

var get_course_filter_from_url = function get_course_filter_from_url() {
  ...
  return false;
}; //Hides dropdown menu when category is clicked on Mobile


jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCardFilter__menu').children().click(function (e) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCardFilter__menu').foundation('close');
}); // shows and hides filtered items

var callback_set_visible_class = function callback_set_visible_class(element, visible) {
  if (visible) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(element).removeClass('programCard--hidden').addClass('programCard--visible');
  } else {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(element).toggleClass('programCard--visible');
  }
};

var callback_set_display_prop = function callback_set_display_prop(element) {
...
var animate_elements_in = function animate_elements_in(elements) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#course_list_more').hide();
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.each(elements, function (k, v) {
    foundation_sites__WEBPACK_IMPORTED_MODULE_2__["default"].Motion.animateIn(elements[k], 'scale-in-up', callback_set_display_prop(elements[k]));
  });
};

var show_all_programs = function show_all_programs() {
  var elements = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCard');
  animate_elements_in(elements);
};

var show_matched_programs = function show_matched_programs(value) {
  var elements = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCard').filter('.' + value); //elements.show();

  animate_elements_in(elements);
};

var hide_unmatched_programs = function hide_unmatched_programs(value) {
  var elements = jquery__WEBPACK_IMPORTED_MODULE_0___default()(".programCard:visible").not('.' + value);

  if (elements.length > 0) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.each(elements, function (k, v) {
      foundation_sites__WEBPACK_IMPORTED_MODULE_2__["default"].Motion.animateOut(elements[k], 'scale-out-down', callback_set_display_prop(elements[k]));
    });
    setTimeout(function () {
      show_matched_programs(value);
    }, 0);
  } else {
    show_all_programs();
  }
...
var set_active_filter_button = function set_active_filter_button(filter) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCardFilter__menuButton[data-filter=' + filter + ']').parent('li').siblings().children('a').removeClass('is-active');
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCardFilter__menuButton[data-filter=' + filter + ']').addClass('is-active');
};

jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).ready(function ($) {
  var courseFilter = get_course_filter_from_url();

  if (courseFilter) {
    hide_unmatched_programs(courseFilter);
    set_active_filter_button(courseFilter);
  }
});
jquery__WEBPACK_IMPORTED_MODULE_0___default()(".programCardFilter__menuButton").click(function () {
  ...
  if (value === "all") {
    ...
    hide_unmatched_programs(value);
  }
}); // changes active class on filter buttons

jquery__WEBPACK_IMPORTED_MODULE_0___default()('.programCardFilter__menuButton').click(function () {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).parent('li').siblings().children('a').removeClass('is-active');
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).addClass('is-active');
}); // Floating Labels for Form Inputs

jquery__WEBPACK_IMPORTED_MODULE_0___default()('.formInput--floatingLabel input, .formInput--boldOutline textarea').focusin(function () {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).parent().addClass('has-value');
});
jquery__WEBPACK_IMPORTED_MODULE_0___default()('.formInput--floatingLabel input, .formInput--boldOutline textarea').blur(function () {
  if (!jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).val().length > 0) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).parent().removeClass('has-value');
  }
}); //API and Backup Database Related Code
//TODO: Update Recruit Code to work as a Node Module
//Cookie Oven Start
...
}; //Recruit Bridge Start

/**
 *
 * Take the submitted values on Doane Landing Pages, parse into a JSON object,
 * submit the object to CRM. If CRM fails, write to database.
 *
 */
//Set the CRM endpoint to use


var endpoint_url = ''; //Model the JSON object with required properties.
...
var json_record = {
  ...
  email: '',
  location: 'ONLIN',
  academic_program: 'ND.PBAC',
  gender: 'U',
  birth_date: '1/1/1901',
  address: '303 N. 52nd St.',
  city: 'Lincoln',
  state: 'NE',
  zip: '68504',
  anticipated_entry_term: 'TBD',
  origin_source: 'OLA Microsite',
  inquiry_source: 'OLA Microsite',
  owner: '',
  territory: 'Default',
  admit_type: '',
  comments: ''
}; //Receive form_data as input, process into correct JSON fields

var form_to_json = function form_to_json(form_data) {
  json_record.first_name = form_data.f_name;
  json_record.last_name = form_data.l_name;
  json_record.cell_phone = form_data.phone;
  json_record.email = form_data.email;

  if (typeof form_data.contactMessage !== 'undefined') {
    json_record.comments = form_data.contactMessage;
  }
}; //Use data from the form to set the Inquiry owner and Admit Type


var set_owner_admitType = function set_owner_admitType(form_data) {
  if (form_data.location === 'CRETE') {
    json_record.owner = '';
    json_record.admit_type = 'First-Time Freshman';
    json_record.address = '1014 Boswell Ave';
    json_record.city = 'Crete';
    json_record.state = 'NE';
    json_record.zip = '68333';
  } else {
    json_record.owner = 'Enrollment Team';
    json_record.admit_type = 'No Transfer Credits';
  }
};

var set_source_code = function set_source_code(cookieName) {
  var source = checkCookie(cookieName);

  if (source) {
    //console.log(source + ': source code set from cookie.');
    json_record.origin_source = source;
    json_record.inquiry_source = source;
    return true;
  } else {
    return false;
  }
}; //Send the Inquiry to the CRM via AJAX


var send_inquiry = function send_inquiry() {
  var rec_data = {
    'record_data': JSON.stringify(json_record)
  };
  jQuery.ajax({
    url: endpoint_url,
    type: 'POST',
    ...
    complete: function complete(xhr, textStatus) {
      ...
      inquiry_catch(rec_data);
    },
    success: function success(data, textStatus, xhr) {
      console.log(data + ': ' + textStatus);

      if (data.toLowerCase() !== "success") {//console.log("Recruit save failed, going to backup DB");
        // inquiry_catch(rec_data);
      } else {//console.log("Recruit save complete");
          //Temporaryily saving to both DBs
          // inquiry_catch(rec_data);
        }
    },
    error: function error(xhr, textStatus, errorThrown) {
      console.log(errorThrown + ': ' + textStatus);
      email_error(rec_data); // inquiry_catch( rec_data );
    }
  });
}; //In case the CRM API throws an error, catch the Inquiry.


var inquiry_catch = function inquiry_catch(record_data) {
  jQuery.ajax({
    url: 'inc/inquiry_database.php',
    type: 'POST',
    dataType: 'json',
    data: record_data,
    complete: function complete(xhr, textStatus) {//console.log( 'Sent to PHP' );
    },
    success: function success(data, textStatus, xhr) {//console.log( data + ': ' + textStatus );
    },
    error: function error(xhr, textStatus, errorThrown) {//console.log( errorThrown + ': ' + textStatus );
      //console.log( record_data );
    }
  });
}; // When Recruit has an error, send an email


var email_error = function email_error(record_data) {
  jQuery.ajax({
    url: 'inc/fail_email.php',
    type: 'POST',
    dataType: 'json',
    data: record_data,
    complete: function complete(xhr, textStatus) {
    ...
    success: function success(data, textStatus, xhr) {
      console.log(data + ': ' + textStatus + ' - Email Success');
    },
    error: function error(xhr, textStatus, errorThrown) {
      console.log(errorThrown + ': ' + textStatus + ' - Email Failure'); //console.log( xhr.responseText );
    }
  });
};
/**
...
jquery__WEBPACK_IMPORTED_MODULE_0___default.a.fn.formToObject = function (options) {
  options = jquery__WEBPACK_IMPORTED_MODULE_0___default.a.extend({}, options);
  var self = this,
      json = {},
      push_counters = {},
      patterns = {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default.a.each(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).serializeArray(), function () {
    // skip invalid keys
    ...
    json = jquery__WEBPACK_IMPORTED_MODULE_0___default.a.extend(true, json, merge);
  });
  return json;
};
/**
 *
 * Package everything together into the King Function
 *
 */


var process_inquiry = function process_inquiry(form_selector) {
  var form_data = jquery__WEBPACK_IMPORTED_MODULE_0___default()(form_selector).formToObject();
  form_to_json(form_data);
  set_owner_admitType(form_data);
  set_source_code('rSourceCookie');
  send_inquiry();
};

var submit_button_disable = function submit_button_disable(bool) {
  if (bool) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.button[type=submit]').html('Submitting').prop('disabled', true);
  } else {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.button[type=submit]').html('Submit').prop('disabled', false);
  }
}; //Submit form to CRM if Valid


jquery__WEBPACK_IMPORTED_MODULE_0___default()('.contactForm__form').on("forminvalid.zf.abide", function (ev, frm) {//console.log("Form id " + ev.target.id + " is invalid");
}).on("formvalid.zf.abide", function (ev, frm) {
  //console.log("Form id " + frm.attr('id') + " is valid");
  //Hide the form
  foundation_sites__WEBPACK_IMPORTED_MODULE_2__["default"].Motion.animateOut(jquery__WEBPACK_IMPORTED_MODULE_0___default()('.contactForm__form'), 'slide-out-right'); //Show the Thank You

  foundation_sites__WEBPACK_IMPORTED_MODULE_2__["default"].Motion.animateIn(jquery__WEBPACK_IMPORTED_MODULE_0___default()('.contactForm__thanks'), 'slide-in-left'); // ajax post form

  process_inquiry('.contactForm__form');
...
module.exports = __webpack_require__(/*! C:\Users\chetg\devprojects\ola_frontend\src\assets\js\app.js */"./src/assets/js/app.js");
